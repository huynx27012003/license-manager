  ...
  module EE
    ...
    module ProtectedMethods
      class Protector
        ...
        def add_singleton_method(method_name, entitlements:)
          ...
          protection_module.define_method(method_name) do |*args, **kwargs, &block|
            raise ProtectedMethodError, "Calling #{name}.#{method_name} is not available in AtLicense CE. Please upgrade to AtLicense EE." if
              AtLicense.ce?

            raise ProtectedMethodError, "Calling #{name}.#{method_name} is not allowed. Please upgrade AtLicense EE." unless
              AtLicense.ee { |key, lic| lic.valid? && key.valid? && key.entitled?(*entitlements) }
        ...
        def add_instance_method(method_name, entitlements:)
          ...
          protection_module.protected_method_names << method_name
          protection_module.define_method(method_name) do |*args, **kwargs, &block|
            raise ProtectedMethodError, "Calling #{self.class.name}##{method_name} is not available in AtLicense CE. Please upgrade to AtLicense EE." if
              AtLicense.ce?

            raise ProtectedMethodError, "Calling #{self.class.name}##{method_name} is not allowed. Please upgrade AtLicense EE." unless
              AtLicense.ee { |key, lic| lic.valid? && key.valid? && key.entitled?(*entitlements) }
      ...
      def self.[](*methods, singleton_methods: methods, instance_methods: methods, entitlements: [])
        ...
        Module.new do
          next unless
            AtLicense.console? # we only want to run protections in console
