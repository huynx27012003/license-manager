  ...
  module EE
    module SSO
      ...
      def redeem_code(code:)
        ...
        raise AtLicense::Error::InvalidSingleSignOnError.new("bad code: #{e.message}", code: "SSO_#{e.error.upcase}")
      end
      ...
      def decrypt_state(ciphertext, secret_key:)
        ...
        raise AtLicense::Error::InvalidSingleSignOnError.new("bad state: #{e.message}", code: 'SSO_STATE_INVALID')
      end
      ...
      def derive_key(secret_key)
        at_license = ActiveSupport::KeyGenerator.new(secret_key)
        salt   = 'sso'.freeze

        at_license.generate_key(salt, 32)
      end
    end
  end
end
