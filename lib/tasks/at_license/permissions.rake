  ...
  namespace :permissions do
    task :add, %i[type] => %i[environment] do |_, args|
      ...
      records.find_each(batch_size:) do |record|
        ...
        unless record.default_permissions?(except: new_permissions, with: prev_permissions)
          AtLicense.logger.info { "Skipping #{record.id}..." }
        ...
        if next_permissions.any?
          ...
          AtLicense.logger.info { "Adding #{diff_permissions.join(',')} permissions to #{record.id}..." }

          record.update!(
            permissions: next_permissions,
          )
        else
          AtLicense.logger.info { "Nothing to add to #{record.id}..." }
        end

        sleep batch_wait
      rescue ActiveRecord::RecordInvalid => e
        AtLicense.logger.info { "Failed #{record.id}: #{e.message}" }
      end
    end
    ...
    namespace :admins do
      ...
      task add: %i[environment] do |_, args|
        batch_size = ENV.fetch('BATCH_SIZE') { 1_000 }.to_i
        task       = Rake::Task['at_license:permissions:add']
        ...
        AtLicense.logger.info { "Adding #{permissions.join(',')} permissions to #{admins.count} admins..." }
    ...
    namespace :environments do
      ...
      task add: %i[environment] do |_, args|
        ...
        AtLicense.logger.info { "Adding #{permissions.join(',')} permissions to #{environments.count} environments..." }
    ...
    namespace :products do
      ...
      task add: %i[environment] do |_, args|
        ...
        AtLicense.logger.info { "Adding #{permissions.join(',')} permissions to #{products.count} products..." }
    ...
    namespace :licenses do
      ...
      task add: %i[environment] do |_, args|
        ...
        AtLicense.logger.info { "Adding #{permissions.join(',')} permissions to #{licenses.count} licenses..." }
    ...
    namespace :users do
      ...
      task add: %i[environment] do |_, args|
        ...
        AtLicense.logger.info { "Adding #{permissions.join(',')} permissions to #{users.count} users..." }
