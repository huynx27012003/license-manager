...
namespace :at_license do
  desc 'Setup AtLicense and the environment'
  task setup: %i[environment db:schema:load db:migrate db:seed] do |_, args|
    ...
    ActiveRecord::Base.logger.silence do
      edition = (args.extras[0] || ENV.fetch('AT_LICENSE_EDITION') { 'CE' }).upcase
      mode    = (args.extras[1] || ENV.fetch('AT_LICENSE_MODE')    { 'singleplayer' }).downcase
      config  = {}
      ...
      config['AT_LICENSE_EDITION'] = edition
      config['AT_LICENSE_MODE']    = mode
      config['AT_LICENSE_HOST']    = ENV.fetch('AT_LICENSE_HOST') {
      ...
      when 'EE'
        ...
        config['AT_LICENSE_LICENSE_FILE_PATH'] = '/etc/at-license/ee.lic'
        config['AT_LICENSE_LICENSE_KEY']       = ENV.fetch('AT_LICENSE_LICENSE_KEY') {
      ...
      when 'singleplayer'
        ...
        id = ENV.fetch('AT_LICENSE_ACCOUNT_ID') {
          print 'Choose an account ID (leave blank for default): '
          gets
        }

        email = ENV.fetch('AT_LICENSE_ADMIN_EMAIL') {
          print 'Choose a primary email: '
          gets
        }

        password = ENV.fetch('AT_LICENSE_ADMIN_PASSWORD') {
        ...
        config['AT_LICENSE_ACCOUNT_ID'] = account.id
      when 'multiplayer'
