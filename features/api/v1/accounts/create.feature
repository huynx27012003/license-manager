...
Feature: Create account
          ...
          "relationships": {
            ...
            "admins": {
              "data": [
                {
                  "type": "user",
                  "attributes": { "firstName": "John", "lastName": "Doe", "email": "john.doe@atlicense.example", "password": "secret" }
                }
              ]
            }
          }
  ...
  Scenario: Anonymous creates an account with autogenerated account information
    When I send a POST request to "/accounts" with the following:
      ...
      """
    Then the response status should be "201"
    And the response body should be an "account" with the slug "anomalyco"
    And the response body should be an "account" with the name "atlicense.vn"
    And the account should receive a "prompt-for-first-impression" email
    And the account "anomalyco" should have 1 "admin"

  Scenario: Anonymous creates an account with autogenerated account information using a domain that is already used
    Given there exists an account "anomalyco"
    When I send a POST request to "/accounts" with the following:
      """
      {
        "data": {
              ...
              "data": [
                {
                  ...
                  "attributes": {
                    "firstName": "Larry",
                    "lastName": "Page",
                    "email": "missingatsymbol.atlicense.vn",
                    "password": "secret123"
                  }
                }
              ]
