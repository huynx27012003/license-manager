  ...
  def perform
    ...
    processes.unordered.find_each do |process|
      ...
      unless process.update(heartbeat_jid: jid)
        AtLicense.logger.warn { "[process.heartbeat.cull] failed to attach: process_id=#{process.id} jid=#{jid}" }
      end

      unless job.perform_async(process.id)
        AtLicense.logger.warn { "[process.heartbeat.cull] failed to queue: process_id=#{process.id} jid=#{jid}" }
      end
    end
  end
end
