class PruneReleaseDownloadLinksWorker < BaseWorker
  ...
  def perform
    ...
    AtLicense.logger.info "[workers.prune-release-download-links] Starting: accounts=#{accounts.count} time=#{cutoff_time}"
    ...
    accounts.unordered.find_each do |account|
      ...
      AtLicense.logger.info "[workers.prune-release-download-links] Pruning #{total} rows: account_id=#{account_id} batches=#{batches}"
      ...
      loop do
        ...
        AtLicense.logger.info "[workers.prune-release-download-links] Pruned #{sum}/#{total} rows: account_id=#{account_id} batch=#{batch}/#{batches}"
    ...
    AtLicense.logger.info "[workers.prune-release-download-links] Done"
  end
end
