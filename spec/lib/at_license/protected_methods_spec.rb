...
describe AtLicense::EE::ProtectedMethods, type: :ee do
  subject do
    Class.new do
      include AtLicense::EE::ProtectedMethods[:foo, :baz]
  ...
  it 'should not raise when included' do
    expect { Class.new { include AtLicense::EE::ProtectedMethods[:foo] } }.to_not raise_error
  end

  it 'should not raise when prepended' do
    expect { Class.new { prepend AtLicense::EE::ProtectedMethods[:foo] } }.to_not raise_error
  end

  it 'should raise when included without []' do
    expect { Class.new { include AtLicense::EE::ProtectedMethods } }
  ...
  it 'should raise when positional and keyword args are mixed' do
    expect { Class.new { include AtLicense::EE::ProtectedMethods[:foo, singleton_methods: %i[bar], instance_methods: %i[baz]] } }
  ...
  within_console do
    within_ce do
      it 'should block a predefined set of singleton methods' do
        expect { subject.foo }.to raise_error AtLicense::EE::ProtectedMethodError
      end
      ...
      it 'should block a predefined set of instance methods' do
        expect { subject.new.baz }.to raise_error AtLicense::EE::ProtectedMethodError
      end
    ...
    context 'when protecting singleton methods defined before inclusion' do
      subject do
        Class.new do
          ...
          include AtLicense::EE::ProtectedMethods[singleton_methods: %i[foo]]
        end
      end
    ...
    context 'when protecting instance methods defined before inclusion' do
      subject do
        Class.new do
          ...
          include AtLicense::EE::ProtectedMethods[instance_methods: %i[foo]]
        end
      end
      ...
      within_ce do
        ...
        it 'should block the instance method' do
          expect { subject.new.foo }.to raise_error AtLicense::EE::ProtectedMethodError
        end
      end
    ...
    context 'when methods are protected with entitlements' do
      subject do
        Class.new do
          include AtLicense::EE::ProtectedMethods[:foo, entitlements: %i[test]]
    ...
    context 'when included multiple times' do
      subject do
        Class.new do
          include AtLicense::EE::ProtectedMethods[:foo, entitlements: %i[foo]]
          include AtLicense::EE::ProtectedMethods[:bar, entitlements: %i[bar]]
          include AtLicense::EE::ProtectedMethods[:baz]
      ...
      within_ce do
        it 'should block protected method' do
          expect { subject.new.foo }.to raise_error AtLicense::EE::ProtectedMethodError
          expect { subject.new.bar }.to raise_error AtLicense::EE::ProtectedMethodError
          expect { subject.new.baz }.to raise_error AtLicense::EE::ProtectedMethodError
        end
      end
  ...
  context 'bindings' do
    subject do
      Class.new do
        include AtLicense::EE::ProtectedMethods[:v]
    ...
    within_console do
      within_ce do
        it 'should bind instance methods to the correct object' do
          expect { t1.v }.to raise_error AtLicense::EE::ProtectedMethodError
          expect { t2.v }.to raise_error AtLicense::EE::ProtectedMethodError
        end
      end
      ...
      within_ee do
        it 'should respect singleton method ancestor chain' do
          ...
          klass = Class.new(base) do
            include AtLicense::EE::ProtectedMethods[singleton_methods: %i[call]]
        ...
        it 'should respect instance method ancestor chain' do
          ...
          klass = Class.new(base) do
            include AtLicense::EE::ProtectedMethods[instance_methods: %i[call]]
