...
describe AtLicense::EE::LicenseFile, type: :ee do
  ...
  before do
    stub_const('AtLicense::PUBLIC_KEY', public_key)
  end
    ...
    context 'when a valid license file exists' do
      with_file path: described_class::DEFAULT_PATH, fixture: 'valid.lic' do
        context 'when a valid license key is used' do
          with_env AT_LICENSE_LICENSE_KEY: license_key do
            ...
            it 'should raise if clock is desynchronized' do
              ...
              with_time lic.issued - 1.minute do
                ...
                expect { lic.valid? }.to raise_error AtLicense::EE::InvalidLicenseFileError
              end
            end
          end
        end
        ...
        context 'when an invalid license key is used' do
          with_env AT_LICENSE_LICENSE_KEY: 'TEST-INVALID' do
          ...
          with_env AT_LICENSE_LICENSE_KEY: license_key do
            ...
            it 'should raise outside grace period' do
              ...
              with_time lic.expiry + 2.months do
                expect { lic.valid? }.to raise_error AtLicense::EE::ExpiredLicenseFileError
              end
            end
  ...
  context 'when using a custom license file path' do
    context 'when using a real relative path' do
      with_env AT_LICENSE_LICENSE_FILE_PATH: file_fixture('valid.lic').relative_path_from(Rails.root), AT_LICENSE_LICENSE_KEY: license_key do
        it 'should be a valid license file' do
          lic = described_class.current
    ...
    context 'when using a relative path' do
      with_file path: '.ee.lic', fixture: 'valid.lic' do
        with_env AT_LICENSE_LICENSE_FILE_PATH: '.ee.lic', AT_LICENSE_LICENSE_KEY: license_key do
          it 'should be a valid license file' do
            lic = described_class.current
    ...
    context 'when using an absolute path' do
      with_file path: '/etc/licenses/ee.lic', fixture: 'valid.lic' do
        with_env AT_LICENSE_LICENSE_FILE_PATH: '/etc/licenses/ee.lic', AT_LICENSE_LICENSE_KEY: license_key do
          it 'should be a valid license file' do
            lic = described_class.current
    ...
    context 'when using an invalid path' do
      with_env AT_LICENSE_LICENSE_FILE_PATH: '/dev/null', AT_LICENSE_LICENSE_KEY: license_key do
        it 'should fail to load license file' do
          lic = described_class.current
  ...
  context 'when using an encoded license file' do
    with_env AT_LICENSE_LICENSE_FILE: license_file, AT_LICENSE_LICENSE_KEY: license_key do
      it 'should be a valid license file' do
        lic = described_class.current
